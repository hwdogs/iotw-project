...
function askCode() {
  if (isEmailValid.value) {
    ...
    get(`/api/customer/ask-code?email=${editFormData.email}&type=register`, () => {
...
const submitAddForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid) => {
    if (valid) {
      ...
      post('/api/customer/add', requestData, () => {
...
const handleUpdate = (id: number) => {
  ...
  const currentRow = tableData.value.find(item => {
    ...
    console.log('item.customerId类型:', typeof item.customerId)
    console.log('传入id类型:', typeof id)
    console.log('比较结果:', Number(item.customerId) === id)
    return Number(item.customerId) === id
  })
  console.log('找到的currentRow:', currentRow)
  if (currentRow) {
    editFormData.customerId = id
  ...
  await formEl.validate((valid) => {
    if (valid) {
      ...
      const requestData: Record<string, any> = {
        customerId: editFormData.customerId  // 确保id字段被包含在请求数据中
      }
      const originalData = tableData.value.find(item => Number(item.customerId) === editFormData.customerId)
      ...
      post('/api/customer/update', requestData, () => {
...
const handleDelete = (id: number) => {
  ElMessageBox.confirm('确认删除该顾客？', '警告', {
    confirmButtonText: '确认',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    get(`/api/customer/delete?id=${id}`, () => {
...
<template>
  <div class="customer-management" style="width: auto;">
    <!--搜索表单-->
    <el-form
        ref="queryForm"
        :inline="true"
      ...
      <div class="form-row">
        ...
        <el-form-item label="注册时间">
          <el-date-picker
              v-model="conditionForm.startRegisterTime"
              type="datetime"
              placeholder="开始时间"
          ...
          <span style="margin: 0 8px;">—</span>
          <el-date-picker
              v-model="conditionForm.endRegisterTime"
              type="datetime"
              placeholder="结束时间"
              value-format="YYYY-MM-DD HH:mm:ss"
          />
      ...
      <div class="form-row">
        ...
        <el-form-item label="排序方向">
          <el-button-group>
            <el-button
                ...
                @click="conditionForm.sortAsc = true; getCustomerData()"
            >
              升序
            </el-button>
            <el-button
                :type="!conditionForm.sortAsc ? 'primary' : ''"
                @click="conditionForm.sortAsc = false; getCustomerData()"
            >
              降序
            </el-button>
          </el-button-group>
        ...
        <el-form-item>
          <el-button size="large" type="primary" :icon="Search" @click="getCustomerData" round>搜索</el-button>
    ...
    <div class="action-buttons">
      <el-button type="primary" :icon="Plus" plain @click="handleAdd">新增顾客</el-button>
    </div>
        ...
        <el-table-column label="操作" width="155" fixed="right">
          <template #default="{ row }">
            <el-button
                ...
                text
                bg
                @click="handleUpdate(row.customerId)"
            >
              编辑
            </el-button>
            <el-button
                ...
                text
                bg
                @click="handleDelete(row.customerId)"
            >
              删除
            </el-button>
          </template>
    ...
    <el-dialog
        ...
        :title="editFormData.customerId ? '编辑顾客信息' : '添加新顾客'"
        width="500px"
        draggable
        @close="handleClose"
    >
      ...
      >
        ...
        <el-form-item label="密码" prop="password" v-if="!editFormData.customerId">
          <el-input v-model="editFormData.password" type="password" placeholder="请输入密码"/>
        </el-form-item>

        <el-form-item label="确认密码" prop="passwordConfirm" v-if="!editFormData.customerId">
        ...
        <el-form-item label="验证码" prop="code" v-if="!editFormData.customerId">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary"
                     @click="editFormData.customerId ? submitEditForm(editForm) : submitAddForm(editForm)">
            {{ editFormData.customerId ? '确认编辑' : '确认添加' }}
          </el-button>
        </span>
      </template>
    </el-dialog>
